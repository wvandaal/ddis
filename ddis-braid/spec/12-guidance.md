...
### §12.11 Dynamic Methodology Projection

> **Purpose**: Transform the static methodology instructions in AGENTS.md into a live
> projection of the store state. The seed tells agents WHAT to work on (task-relevant
> context); the dynamic methodology tells them HOW to work (adapted to current gaps,
> ceremony level, and available infrastructure). Together: complete agent orientation.
>
> **Traces to**: SEED.md §7 (Self-Improvement Loop), §8 (Interface Principles),
> C7 (Self-Bootstrap)

#### INV-GUIDANCE-022: Dynamic Methodology Section in AGENTS.md

**Traces to**: SEED.md §7, C7 (self-bootstrap), INV-GUIDANCE-021
**Type**: Invariant
**Stage**: 1
**Statement**: `braid seed --inject AGENTS.md` generates a `<braid-methodology>`
section at the TOP of the file (before static content, maximizing k* position)
containing live store-derived methodology guidance:

```
<braid-methodology>
<!-- Generated by braid. Regenerate: braid seed --inject AGENTS.md -->

## Methodology Gaps
- 7 observations with uncrystallized spec IDs → braid spec create
- 12 tasks with unresolved spec refs → crystallize first
- 3 current-stage INVs untested → add L2+ witness

## Ceremony Protocol (current k*={k_eff:.1})
{if k_eff > 0.7}
...
{end}
For known-category bug fixes: execute-first OK if provenance chain
(code → observation → spec element) exists after commit.

## Next Actions (R(t) pre-computed)
1. [P1] t-xxxx "Fix merge cascade" (impact=0.72) → braid go t-xxxx
2. [P1] t-yyyy "Implement AVET" (impact=0.65) → braid go t-yyyy
3. [P2] t-zzzz "Add witness CLI" (impact=0.51) → braid go t-zzzz

## Session Constraints
- WITNESS system: NOT YET IMPLEMENTED (spec only)
- LIVE index: IMPLEMENTED (Store::live_value available)
- .cache/ persistence: IMPLEMENTED
- Adaptive harvest: {if AGP implemented}ACTIVE{else}PENDING{end}
</braid-methodology>
```

**Key properties**:
1. **Position**: TOP of file, before static content (maximum k* exposure)
2. **Derived**: Every field computed from store state, not hardcoded
3. **Idempotent**: Regenerating produces same output for same store state
4. **Concise**: ≤ 200 tokens (ambient context, not active)

**Falsification**: The `<braid-methodology>` section contains hardcoded values
(e.g., "3 gaps" when the store has 7), or is absent after `braid seed --inject`.
**Verification**: V:PROP

---

#### INV-GUIDANCE-023: Provenance Chain Completeness

**Traces to**: C5 (Traceability), FM-035 (Ideation-to-Task Skip)
**Type**: Invariant
**Stage**: 1
**Statement**: For every code change committed to the repository, there exists
a provenance chain: code → observation/task → spec element. The chain need not
be created in execution order (execute-first is OK for known-category fixes),
but it must EXIST after the commit.

```
∀ commit C with modified_files(C) ∩ source_files ≠ ∅:
  ∃ observation O or task T in store with:
    O.tx.wall_time ≈ C.timestamp (within session) ∧
    O.body references a spec element ID ∈ spec/*.md
```

**Detection**: `braid trace --since <commit>` scans changed files for spec
references. If none found, emits a provenance gap signal.

**Ceremony adaptation**: The required ceremony level depends on the change type:
- **Known-category bug fix**: execute → observe → reference (minimum)
- **Feature implementation**: observe → task → execute (standard)
- **Novel design**: observe → crystallize → task → execute (full)

The `<braid-methodology>` section communicates the current ceremony level
so agents know what's expected.

**Falsification**: A commit modifies `store.rs` but no observation or task
in the store references a spec element related to the change, and no
provenance gap signal was emitted.
**Verification**: V:PROP
...
##### Problem
Static methodology instructions in AGENTS.md don't adapt to store state.
Agents read "always crystallize before implementing" but at low k* they
can't follow it. The gap dashboard shows methodology violations but only
when agents run `braid status` — which they may not do.

##### Options
A) **Static instructions** (current) — same text every session. Agents forget
   or ignore under k* pressure.
B) **Dynamic methodology section** — generated from store state, positioned at
   top of AGENTS.md for maximum k* exposure. Includes current gaps, adapted
   ceremony level, pre-computed next actions, and session constraints.

##### Decision
**Option B.** The methodology section is a live projection of the store,
just as the seed section is a live projection of task-relevant context.
The two together form complete agent orientation: HOW (methodology) + WHAT (seed).

The position at the TOP of the file ensures it's read first, with maximum
attention budget. The concise format (≤200 tokens) keeps it in ambient
context territory — it shapes behavior without competing for k*.

##### Falsification
The dynamic section is too large (>300 tokens) and becomes a k* sink rather
than ambient context. Mitigation: strict 200-token budget with the same
compression ladder as the guidance footer (INV-BUDGET-004).
