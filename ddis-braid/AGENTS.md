...
<!-- Updated: 1774297431 | Store: 68883 datoms -->

## Methodology Gaps
- 66 observations with uncrystallized spec IDs → braid spec create
- 11 tasks with unresolved spec refs → crystallize first
- 243 current-stage INVs untested → add L2+ witness
- concentration: 4 traces in BILATERAL — Review: braid task search INV-BILATERAL
- concentration: 3 traces in FOUNDATION — Review: braid task search INV-FOUNDATION
...
1. "ACP-DRY-1: Extract novelty_from_count(N) -> f64 to budget.rs" (impact=1.24) → braid go t-3d5e9546
2. "RDI-5: Precedent query — find_resolution_precedent(store, " (impact=0.71) → braid go t-d9536d87
3. "ATT-2-IMPL: Hebbian feedback from verbose-request detection" (impact=0.27) → braid go t-065fdbaa

</braid-methodology>

<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1774297431 | Store: 68883 datoms, 6816 entities -->
...
Next actions:
  1. Work — R(t) top: "ACP-DRY-1: Extract novelty_from_count(N) -> f64 to budget.rs. Replace 3 duplicate 1/sqrt(N) formulas in budget.rs:682, context.rs:282, routing.rs:894 with single function. Algebraic property: monotonically decreasing in N, 1.0 at N=0, approaches 0 as N->inf. FILE: crates/braid-kernel/src/budget.rs + routing.rs + context.rs" (impact=1.24) — t-3d5e9546
     run: braid go t-3d5e9546
...
Active intentions (3 tasks):
  ...
  [t-3d5e9546] ACP-DRY-1: Extract novelty_from_count(N) -> f64 to budget.rs. Replace 3 duplicate 1/sqrt(N) formulas in budget.rs:682, context.rs:282, routing.rs:894 with single function. Algebraic property: monotonically decreasing in N, 1.0 at N=0, approaches 0 as N->inf. FILE: crates/braid-kernel/src/budget.rs + routing.rs + context.rs (task, P1/high, in-progress)
    Blocks: ACP-SCORE-1: Default AcquisitionScore for all context blocks. Add ContextBlock::new_scored(precedence, content, tokens, kind, impact) constructor that computes baseline score from precedence level and token cost. Replace 96 attention:None constructors across 15 files with scored defaults. Eliminates the None/Some bifurcation — all blocks participate in UAQ ranking. FILE: crates/braid-kernel/src/budget.rs + all command files (t-238ea707)
