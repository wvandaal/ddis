...
<!-- Updated: 1774225507 | Store: 49108 datoms -->
...
1. "EXTRACTOR-2: braid extract command — query :extractor/* en" (impact=0.81) → braid go t-9411936c
2. "SUBSTRATE-3: Decouple agent_name_from_files — remove .rs s" (impact=0.71) → braid go t-bc7d87d5
3. "EVIDENCE-R3: Unit + proptest for evidence-weighted boundarie" (impact=0.70) → braid go t-42a1a89b

</braid-methodology>

<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1774225507 | Store: 49108 datoms, 5154 entities -->
...
Next actions:
  1. Work — R(t) top: "EXTRACTOR-2: braid extract command — query :extractor/* entities, invoke commands, validate output datoms, update :extractor/last-run (INV-FOUNDATION-003). FILE: crates/braid/src/commands/extract.rs" (impact=0.81) — t-9411936c
     run: braid go t-9411936c

Protocol: observe → status → observe → harvest --commit | seed --inject AGENTS.md

Active intentions (5 tasks):
  [t-bc7d87d5] SUBSTRATE-3: Decouple agent_name_from_files — remove .rs suffix stripping, use generic path stem extraction (INV-FOUNDATION-001). FILE: crates/braid-kernel/src/topology.rs (task, P2/medium, in-progress)
    Traces to: :spec/inv-foundation-001
  ...
  [t-42a1a89b] EVIDENCE-R3: Unit + proptest for evidence-weighted boundaries. TESTS: (1) composite_evidence_weight returns 1.0 for observed+L4+fresh. (2) Returns low value for hypothesized+no-witness+stale. (3) Proptest: weight monotonically non-decreasing as any factor improves. (4) Weighted coverage >= unweighted coverage when high-evidence entities are covered. (5) Backward compatible: store without evidence datoms → weight defaults to 1.0 for all. (6) Proptest: coverage in [0.0, 1.0] for any store state. TRACES TO: INV-FOUNDATION-005, INV-BILATERAL-007. FILE: crates/braid-kernel/src/bilateral.rs (test, P1/high, in-progress)
    Depends on: EVIDENCE-R1: composite_evidence_weight(store, entity) -> f64 function. Computes provenance_weight (ADR-STORE-008: observed=1.0..hypothesized=0.2) × depth_factor (INV-WITNESS-002: L1=0.3, L2=0.6, L3=0.8, L4=1.0, none=0.1) × freshness (ADR-HARVEST-005: exponential decay per namespace). Returns [0.0, 1.0]. No new schema — reads existing :tx/provenance-type, :impl/verification-depth, :observation/timestamp. BACKGROUND: This UNIFIES four existing graduated confidence systems (provenance, witness depth, uncertainty, resolution weight) into a single callable function for boundary checking. TRACES TO: ADR-STORE-008, INV-WITNESS-002, ADR-HARVEST-005, INV-FOUNDATION-005. ACCEPTANCE: (A) observed+L4+fresh entity → weight ≈ 1.0. (B) hypothesized+no-witness+stale entity → weight ≈ 0.02. (C) Weight is monotonically non-decreasing as provenance/depth/freshness improve. FILE: crates/braid-kernel/src/bilateral.rs (t-e31c20ca)
    Traces to: :spec/inv-bilateral-007, :spec/inv-foundation-005
  [t-9411936c] EXTRACTOR-2: braid extract command — query :extractor/* entities, invoke commands, validate output datoms, update :extractor/last-run (INV-FOUNDATION-003). FILE: crates/braid/src/commands/extract.rs (feature, P1/high, in-progress)
    Depends on: EXTRACTOR-1: Register :extractor/* schema — /command, /produces, /schedule, /boundary, /status, /last-run, /language (ADR-FOUNDATION-009). FILE: crates/braid-kernel/src/schema.rs (t-92908e53), SUBSTRATE-1: Register abstract component schema — :component/id, /name, /type, /reachable, /tested, /parent + :composition/from, /to, /type (ADR-FOUNDATION-008). FILE: crates/braid-kernel/src/schema.rs (t-ae46a5c4)
    Blocks: EXTRACTOR-6: Built-in Git extractor — braid extract git populates :component/* for commits, branches, authors + :composition/* for merge edges (ADR-FOUNDATION-007). FILE: crates/braid/src/extractors/git.rs (t-50ae6933), SUBSTRATE-6: Built-in Rust extractor — braid extract rust scans Cargo workspace, populates :component/* datoms for modules/functions, computes :component/reachable via mod.rs analysis (ADR-FOUNDATION-007). FILE: crates/braid/src/extractors/rust.rs (t-86c6892d), EXTRACTOR-3: Meta-extractor — scan project for language signals, transact :extractor/* recommendations (INV-FOUNDATION-004). Detect: Cargo.toml→Rust, package.json→TS, go.mod→Go, requirements.txt/pyproject.toml→Python, pom.xml→Java. FILE: crates/braid/src/commands/init.rs (t-b47c7c79), EXTRACTOR-4: braid extract --verify validates extractor output — checks datoms conform to :extractor/produces namespace, schema types correct, no orphan refs (INV-FOUNDATION-002). FILE: crates/braid/src/commands/extract.rs (t-3a2dd7ce), EXTRACTOR-5: Wire extractor gaps into guidance — when BoundaryCheck has 0% coverage AND :extractor/* entity exists with status=recommended, guidance footer says 'install extractor: <command>' (INV-FOUNDATION-004). FILE: crates/braid-kernel/src/guidance.rs (t-f3061562)
    Traces to: :spec/inv-foundation-003
