...
<!-- Updated: 1774434023 | Store: 89620 datoms -->

## Methodology Gaps
- 85 observations with uncrystallized spec IDs → braid spec create
- 20 tasks with unresolved spec refs → crystallize first
- 262 current-stage INVs untested → add L2+ witness
- concentration: 3 traces in CONVERGENCE — Review: braid task search INV-CONVERGENCE
- concentration: 13 traces in EMBEDDING — Review: braid task search INV-EMBEDDING
...
1. "DOGFOOD-2: Three work sessions with UAQ measurement protocol" (impact=1.39) → braid go t-655e3cf2
2. "RDI-5: Precedent query — find_resolution_precedent(store, " (impact=0.75) → braid go t-d9536d87
3. "WALPHA-1: W-alpha (Working Set Alpha) implementation. Privat" (impact=0.31) → braid go t-56b8f57e

</braid-methodology>

<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1774434023 | Store: 89620 datoms, 8985 entities -->
...
Decisions (settled, do not relitigate):
  - AUDIT DEFECT-001 (MAJOR): innate_concept_datoms() hard-codes HashEmbedder at concept.rs:478. The kernel function makes an embedder policy decision instead of accepting a parameter.

Next actions:
  1. Work — R(t) top: "DOGFOOD-2: Three work sessions with UAQ measurement protocol" (impact=1.39) — t-655e3cf2
     run: braid go t-655e3cf2
...
Active intentions (3 tasks):
  ...
  [t-655e3cf2] DOGFOOD-2: Three work sessions with UAQ measurement protocol (task, P0/critical, in-progress)
    Description: CONTEXT: Three work sessions on the external project to generate calibration data for the UAQ. Each session follows the full braid lifecycle: seed -> work -> observe -> harvest.
    Depends on: DOGFOOD-1: External project setup — init braid on Go CLI with measurement baseline (t-0bee0f20)
    Blocks: DOGFOOD-3: Analysis and verdict — empirical validation of UAQ hypothesis (t-fe646c79)
    Traces to: #6970fa4b…
