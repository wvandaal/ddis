...
<!-- Updated: 1774046360 | Store: 30685 datoms -->

## Methodology Gaps
- 32 observations with uncrystallized spec IDs → braid spec create
- 21 tasks with unresolved spec refs → crystallize first
- 252 current-stage INVs untested → add L2+ witness
- concentration: 4 traces in BILATERAL — Review: braid task search INV-BILATERAL
- concentration: 6 traces in GUIDANCE — Review: braid task search INV-GUIDANCE
- concentration: 7 traces in TASK — Review: braid task search INV-TASK
...
1. "ST-3: Implement session trajectory ring buffer (last 20 sess" (impact=0.26) → braid go t-4a5853fb
2. "TOPO-PLAN: Implement TopologyPlan struct and compute_plan() " (impact=0.23) → braid go t-57c4333f
3. "MCP-EXTRACT-1: Behavioral signal taxonomy — define the 6 s" (impact=0.20) → braid go t-ff6a4143

</braid-methodology>

<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1774046360 | Store: 30685 datoms, 3630 entities -->

### Session Context
Braid: append-only datom store with CRDT merge. Harvest/seed lifecycle makes conversations disposable, knowledge durable.
Braid: append-only datom store (CRDT merge, content-addressed). 30685 datoms, 3630 entities. Codebase: 104570 LOC across 78 .rs files
Key files:
  ddis-braid/crates/braid-kernel/src/guidance.rs (10515 LOC)
  ddis-braid/crates/braid-kernel/src/seed.rs (4993 LOC)
  ddis-braid/crates/braid-kernel/src/query/graph.rs (4546 LOC)
  ddis-braid/crates/braid-kernel/src/schema.rs (4455 LOC)
  ddis-braid/crates/braid-kernel/src/harvest.rs (4270 LOC)
Goal: harvest/seed replaces HARVEST.md. Status: 159 harvests, 339 observations, 130 decisions captured.
...
Next actions:
  1. Work — R(t) top: "ST-3: Implement session trajectory ring buffer (last 20 sessions). BACKGROUND: A ring buffer of session summaries enables cross-session convergence tracking and trajectory-aware seed. Single entity :system/session-trajectory with :trajectory/history containing JSON array. Each entry: started_at, agent, start_fitness, end_fitness, duration, tasks_closed, convergence_rate. APPROACH: (1) Add :trajectory/history attribute (String, stores JSON array). (2) At session start: read current history, append new entry with start state, truncate to 20 entries, write back. (3) At harvest: update last entry with end state (end_fitness, duration, convergence_rate = delta_F / duration). (4) Entity is content-addressed: :system/session-trajectory. ACCEPTANCE: (A) After 3 sessions, trajectory has 3 entries. (B) Each entry has start and end fitness. (C) Ring buffer caps at 20. (D) convergence_rate = (end_fitness - start_fitness) / duration_hours. DEPENDS-ON: ST-1. FILE: crates/braid-kernel/src/guidance.rs + crates/braid/src/commands/harvest.rs. TRACES TO: INV-BILATERAL-001, INV-SEED-001." (impact=0.26) — t-4a5853fb
     run: braid go t-4a5853fb
