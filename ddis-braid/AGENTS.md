...
<!-- Updated: 1774362985 | Store: 78268 datoms -->

## Methodology Gaps
- 74 observations with uncrystallized spec IDs → braid spec create
- 17 tasks with unresolved spec refs → crystallize first
- 254 current-stage INVs untested → add L2+ witness
- concentration: 16 traces in DAEMON — Review: braid task search INV-DAEMON
...
1. "RDI-5: Precedent query — find_resolution_precedent(store, " (impact=0.73) → braid go t-d9536d87
2. "ATT-3-IMPL: Task-conditioned relevance scoring via spec name" (impact=0.29) → braid go t-88751f73
3. "DOGFOOD-1: External project setup — init braid on Go CLI w" (impact=0.28) → braid go t-0bee0f20

</braid-methodology>

<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1774362984 | Store: 78268 datoms, 7615 entities -->
...
Active intentions (3 tasks):
  ...
  [t-30f5d046] PERF-4: Introspective Observer Daemon (ADR-FOUNDATION-031, INV-FOUNDATION-015). The daemon's primary purpose is enabling C7 self-bootstrap by giving braid persistent identity and self-observation capability. Performance (<100ms status) is a consequence, not the goal. ARCHITECTURE: (1) Persistent process with Unix socket (.braid/daemon.sock). Holds Store + indexes + computed state in memory. (2) INTROSPECTIVE OBSERVATIONS: Every command generates :runtime/* datoms in W-alpha — latency-ms, cache-hit, routing-ms, store-datom-count, command-name, error-count. These use the SAME 6 ADR-029 lifecycle attributes as project knowledge connections. (3) REFLEXIVE BOUNDARIES: RuntimeBoundary type in BoundaryRegistry evaluates self-model against measurements. latency > 5s = reflexive divergence. cache-miss-rate > 0.5 = reflexive divergence. routing-cost growing faster than datom-count = reflexive divergence. (4) REFLEXIVE F(S): Include a reflexive component in F(S) — does braid's self-model match actual behavior? (5) CODE-AWARE CRB: Before creating a task, grep codebase for existing implementations. 'store cache' -> find write_index_cache in layout.rs -> 'Already implemented. Create anyway?' (6) SESSION LIFECYCLE: Auto-start on first command, auto-stop on harvest --commit. W-alpha flushed to journal on session end. (7) INCREMENTAL ROUTING: On new txn, re-score only AFFECTED tasks (those with deps/traces to changed entities), not all 256. O(k) instead of O(N). ACCEPTANCE: (A) status with daemon: <100ms. (B) :runtime/* datoms recorded per command. (C) RuntimeBoundary detects latency regression. (D) Code-aware CRB prevents duplicate task creation. (E) FEGH bridge hypotheses work on self-model. (F) Clean shutdown writes cache + flushes W-alpha. DEPENDS-ON: PERF-2. TRACES: ADR-FOUNDATION-031, INV-FOUNDATION-015, ADR-STORE-006, C7. FILE: crates/braid/src/daemon.rs (new) + crates/braid-kernel/src/bilateral.rs (RuntimeBoundary) (task, P1/high, in-progress)
    Depends on: WALPHA-1: W-alpha (Working Set Alpha) implementation. Private agent working set for runtime telemetry (INV-STORE-014). Separate from shared store — NOT included in CRDT merge (t-56b8f57e), WALPHA-1: W-alpha (Working Set Alpha) implementation. Private agent working set for runtime telemetry (INV-STORE-014). Separate from shared store — NOT included in CRDT merge (t-56b8f57e), PERF-3: Incremental transaction loading. Instead of reparsing all 6766 EDN files when cache is stale, load from cache + append only NEW transaction files (those with mtime > cache mtime) (t-b38f580d), LIVESTORE-5: Systematic migration of all commands to LiveStore. CONTEXT: After LIVESTORE-4 validates the pattern on status.rs, migrate the remaining ~25 commands (120+ call sites across 30+ files). The migration is mechanical: replace DiskLayout::open + load_store with LiveStore::open, replace layout.write_tx with live.write_tx, replace &store with live.store() (t-a4a22f27)
    Blocks: ROUTING-INCR-1: Incremental routing for daemon. On new txn, re-score only AFFECTED tasks instead of all 256 (t-83d99b6b), PERF-4a: Capability census at daemon startup. The daemon's FIRST act is binary reflection — scanning compiled-in capabilities and recording them as :capability/* datoms in W-alpha. This grounds the store's model in the binary's reality, preventing the class of error where tasks are created for already-implemented features (t-0986f448), ONTOLOGY-DISCOVER-1: Ontological surprise detection — the daemon watches for unknown unknowns. FEGH detects missing BRIDGES between known clusters (known unknowns). This detects missing CATEGORIES entirely (unknown unknowns) (t-bd434c69), ACP-TRACK-2: Re-enable universal presentation tracking via daemon. ACP-TRACK-1 was descoped to status.rs-only because loading the full store in maybe_inject_footer is O(N) = 30s+. Once the daemon keeps the store hot in memory, universal tracking becomes cheap (t-c0650b3c), EPIC: PERF-4 MVP Daemon — Introspective Observer with Reflexive Runtime Datoms (t-8f71bbb2)
