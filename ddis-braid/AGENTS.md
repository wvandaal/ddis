...
<!-- Updated: 1773864812 | Store: 15055 datoms -->
...
<braid-seed>
<!-- Generated by braid. Do not edit manually. Regenerate: braid seed --inject AGENTS.md -->
<!-- Updated: 1773864812 | Store: 15055 datoms, 2133 entities -->

### Session Context
Braid: append-only datom store (CRDT merge, content-addressed). 15055 datoms, 2133 entities. Codebase: 83524 LOC across 72 .rs files
Key files:
...
Goal: harvest/seed replaces HARVEST.md. Status: 59 harvests, 151 observations, 100 decisions captured.
...
Unharvested: 2 observations (run braid harvest --commit)
  - STRUCTURAL INSIGHT: All 5 dog-food friction points share a common root — the system lacks a formal model of its own observation apparatus. It can observe external state (store, files, git) but cannot ...
  - DEEP ANALYSIS: Dog-food assessment → structural insight extraction. Five friction points each reveal a deeper design principle. Goal: crystallize these into formal spec elements and radically innovati...
...
- :spec/adr-guidance-007 — System 1/System 2 Diagnosis
- :spec/inv-interface-005 (10 attrs)
- :spec/adr-interface-009 (9 attrs)
- :observation/audit-results-for-spec21-witnessmd-5-critical-issues-1 — AUDIT RESULTS for spec/21-witness.md — 5 critical issues: (1) V:ARCH undefined — replace with V:PROP, (2) WITNESS missing from verification plan §16, (3) Layer 5 undefined in schema spec — reclassify ...
- :session/s-1773271079 (4 attrs)
- :observation/session-end-turn-1200-2-agp-test-failures-remain-timing — SESSION END (turn ~1200): 2 AGP test failures remain (timing-related tx_velocity + methodology_gaps). Root cause: test creates datoms with test-now but should_warn_on_exit uses system-now. Fix: propag...
- :observation/task-audit-results-408-tasks-20-categories-issues-found — TASK AUDIT RESULTS (408 tasks, 20 categories): ISSUES FOUND: (1) COF tasks have NO spec refs — INV-GUIDANCE-014 is crystallized in spec/12-guidance.md but COF task titles don't contain the INV ID. Nee...
- :task/t-83013809 (12 attrs)
- :observation/dog-food-observation-session-024-the-task-routing-system — DOG-FOOD OBSERVATION (Session 024): The task routing system (R(t)) combined with braid next/go/close creates a genuinely effective work loop. The EPIC dependency fix was a turning point — going from 1...
- :observation/session-024-6-tasks-closed-3-wave-2-leaf-3-p0-s0-close — Session 024: 6 tasks closed (3 Wave 2 leaf + 3 P0 S0-CLOSE). EPIC dependency semantics fixed — EPICs no longer block children. Ready set: 13→168. New: :resolution/* schema attrs, MergeReceipt frontier...
- :observation/session-022-11-agent-comprehensive-audit-phase-0-fixes — Session 022: 11-agent comprehensive audit + Phase 0 fixes + 228-task self-bootstrap. Key insight: priority inversion (infrastructure over purpose). Three systemic patterns identified: priority inversi...
- :observation/phase-2d-error-rendering-braiderrorrendermode-integrates — Phase 2D error rendering: BraidError.render(mode) integrates errors into the output mode pipeline. JSON mode: structured {error: {what, why, fix, spec_ref}}. Agent mode: three-part navigative (error c...
- :observation/phase-0-1-2ab-4ab-complete-bilateral-json-fix-skip-imple — Phase 0-1-2AB-4AB complete: bilateral JSON fix, --skip implementation, POSIX command -v, .braid auto-detect, agent-first piped default, footer dedup (7→1), help text demonstrations, status --deep prog...
- ... and 4 more
