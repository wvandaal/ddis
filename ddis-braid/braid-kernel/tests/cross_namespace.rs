...
fn schema_store_query() {
    ...
    // Genesis schema has exactly 17 axiomatic attributes (INV-SCHEMA-002)
    let genesis_attr_count = store.schema().len();
    assert_eq!(
        genesis_attr_count, 17,
        "INV-SCHEMA-002: genesis has 17 attributes"
    );
...
fn bootstrap_store_query_trilateral() {
    ...
    let tx = Transaction::new(a, ProvenanceType::Observed, "Bootstrap spec elements")
        ...
        .assert(
            inv_store_001,
            Attribute::from_keyword(":impl/file"),
            Value::String("braid-kernel/src/store.rs".to_string()),
        )
        // ADR-EAV-001
        .assert(
            adr_eav_001,
