...
- **phi (Store -> Layout)**: `serialize_transaction` and `transaction_hash` are pure functions —
  ...
  `braid-kernel/src/layout.rs` where they participate in the verification surface.

- **psi (Layout -> Store)**: `load_store` reads files from disk and reassembles the Store.
  This is I/O and lives in `braid/src/persistence.rs`.
...
```rust
// braid-kernel/src/layout.rs — pure functions (INV-LAYOUT-011, INV-LAYOUT-001)
...
// braid/src/persistence.rs — I/O (INV-LAYOUT-003, INV-LAYOUT-007, INV-LAYOUT-009)
...
| NEG-LAYOUT-004 | No transport dependencies | `braid-kernel/Cargo.toml` has zero transport-related crates |
...
| NEG-LAYOUT-004 (no transport deps) | `braid-kernel/Cargo.toml` has no git/rsync/scp crates | `grep -r "git2\|libgit\|rsync" braid-kernel/` returns 0 hits |
