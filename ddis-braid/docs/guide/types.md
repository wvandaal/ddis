...
**Notes**: Used by all persistence functions in `crates/braid/src/persistence.rs` and serialization
functions in `crates/braid-kernel/src/layout.rs`. `HashMismatch` supports INV-LAYOUT-005 (integrity
self-verification).
