...
1. **Actionable, not informational.** "Drift score is 0.3" is informational.
   ...
   protocol in docs/guide/00-architecture.md: `{what} -- {why} -- {recovery}`.
...
1. **Specific first-action.** The directive should include not just the task
   ...
   Transaction impl in crates/braid-kernel/src/store.rs" is a directive. The
...
3. **Acceptance criteria should be mechanically verifiable.** "Implement
   ...
   in docs/guide/01-store.md SS1.4 pass, and `cargo test` exits 0" is
...
**First action**: Define the `Transaction<Building>` typestate in `crates/braid-kernel/src/store.rs`.
**Acceptance**: All proptest properties in docs/guide/01-store.md SS1.4 pass. `cargo test` exits 0.
