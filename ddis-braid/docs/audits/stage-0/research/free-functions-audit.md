...
> **Bead**: R5.2a
> **Traces to**: ADR-ARCHITECTURE-001 (planned); docs/guide/00-architecture.md SS0.1 (kernel/binary split)
...
| `merge()` | `impl Store { pub fn merge(&mut self, other: &Store) -> MergeReceipt }` | **MOVE to free function** | Merge is a set-union operation that composes with cascade steps. Spec/01 defines it as a method; docs/guide/07 already defines it as `pub fn merge(target: &mut Store, source: &Store)`. Free function is correct. |
...
(INV-STORE-014). The guide (docs/guide/03-query.md line 37) correctly defines it as a
...
methods. The guide (docs/guide/05-harvest.md) correctly decomposes them into free
...
### 4.1 QUERY Namespace (docs/guide/03-query.md)
...
### 4.2 RESOLUTION Namespace (docs/guide/04-resolution.md)
...
### 4.3 HARVEST Namespace (docs/guide/05-harvest.md)
...
### 4.4 SEED Namespace (docs/guide/06-seed.md)
...
### 4.5 MERGE Namespace (docs/guide/07-merge-basic.md)
...
### 4.6 GUIDANCE Namespace (docs/guide/08-guidance.md)
...
### 4.7 INTERFACE Namespace (docs/guide/09-interface.md)

| Function | Signature | Namespace |
|---|---|---|
| `format_output()` | `pub fn format_output(response: &ToolResponse, mode: OutputMode, footer: &GuidanceFooter) -> String` | crates/braid/src/output.rs (binary crate) |
| `load_store()` | `pub fn load_store(path: &Path) -> Result<Store, PersistenceError>` | crates/braid/src/persistence.rs (binary crate) |
| `save_store()` | `pub fn save_store(store: &Store, path: &Path) -> Result<(), PersistenceError>` | crates/braid/src/persistence.rs (binary crate) |
...
|---|---|---|---|---|---|
| `query()` | spec/01-store.md, spec/03-query.md | `impl Store` | docs/guide/03-query.md | free fn | **Free function** |
| `merge()` | spec/01-store.md, spec/07-merge.md | `impl Store` | docs/guide/07-merge-basic.md | free fn | **Free function** |
| `harvest_detect()` | spec/05-harvest.md | `impl Store` | docs/guide/05-harvest.md | free fn (`harvest_pipeline`) | **Free function** |
| `harvest_commit()` | spec/05-harvest.md | `impl Store` | docs/guide/05-harvest.md | decomposed to free fns | **Decompose** |
| `associate()` | spec/06-seed.md | `impl Store` | docs/guide/06-seed.md | free fn | **Free function** |
| `assemble()` | spec/06-seed.md | `impl Store` | docs/guide/06-seed.md | free fn (`assemble_seed`) | **Free function** |
| `seed()` | spec/06-seed.md | `impl Store` | docs/guide/06-seed.md | free fn (`assemble_seed`) | **Free function** |
...
All resolution functions (5 functions in docs/guide/04-resolution.md): **Correct**.
All guidance functions (7 functions in docs/guide/08-guidance.md): **Correct**.
All harvest helpers (3 functions in docs/guide/05-harvest.md): **Correct**.
All seed helpers (5 functions in docs/guide/06-seed.md): **Correct**.
All interface/persistence functions (3 functions in docs/guide/09-interface.md): **Correct**.
...
| Module (crates/braid-kernel/src/) | Functions |
|---|---|
