...
- **Stage 0**: `ApproxTokenCounter` (chars/4 with content-type heuristic). Zero dependencies.
  Applied to docs/guide/00-architecture.md as the `TokenCounter` trait with `ApproxTokenCounter`
  implementation.
...
The `TokenCounter` trait is defined in docs/guide/00-architecture.md (section 0.6) and will be
implemented in `crates/braid-kernel/src/lib.rs` or a dedicated `crates/braid-kernel/src/budget.rs` module.
