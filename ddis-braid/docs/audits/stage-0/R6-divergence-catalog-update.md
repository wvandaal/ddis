...
Verified via `grep -rn 'SPEC-GAP' docs/guide/ spec/` -- zero hits in docs/guide/ and spec/.

---

## 2. Type Divergence Catalog (docs/guide/types.md)

### V1 Audit Baseline

The V1 audit (14 Fagan inspection subagents) identified:
- **67 spec-guide divergences** (5 CRITICAL) across all categories
- **13 type-level divergences** (D1-D13) tracked in docs/guide/types.md Appendix A
...
### 4.1 Stale SEED INV References in docs/guide/12-stages-1-4.md (FIXED)

The R5.1b agent renumbered SEED INVs (added INV-SEED-004/005, shifted old 004-006 to
006-008) and updated spec/17-crossref.md correctly, but docs/guide/12-stages-1-4.md retained
...
docs/guide/00-architecture.md and referenced in 7+ spec files, but was not tracked in
