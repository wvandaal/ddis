...
| Adding a new element to spec/ | FM-006 | Dependent documents (docs/guide/, crossref, audit) won't update themselves |
...
durable artifact. The manual harvest (docs/HARVEST.md Session 001) captured high-level abstractions
...
implementation guides in docs/guide/ were written during or before this finalization, using a
different numbering scheme. When the spec IDs were settled, the guides were not realigned.
The result: docs/guide/02-schema.md references INV-SCHEMA-003 but describes content that actually
...
elements, verification matrix rows. But no guide file was updated. The docs/guide/09-interface.md
header still says "Stage 0 elements: INV-INTERFACE-001–003 (3 INV)" when the spec now says
5 INV. The docs/guide/10-verification.md says "104 invariants" when the spec says 107.

**Additionally**: The audit discovered that docs/guide/10-verification.md already had stale counts
...
**Root cause**: No dependency tracking between documents. The relationship "docs/guide/09-interface.md
...
INTERFACE namespace, run `braid status`, verify docs/guide/09-interface.md is flagged as stale
...
- docs/guide/10-verification.md was already 2 edits behind (said 62 when should say 64)
- docs/guide/12-stages-1-4.md had stale percentage (59.6% → 59.8%) and stale activation list
...
docs/guide/00-architecture.md, docs/guide/01-store.md, and all worked examples say "BLAKE3."
...
occurrence in spec/, docs/guide/, and src/.
...
**Trigger**: Audit found "62 Stage 0 INVs" in docs/guide/10-verification.md, "64" in
docs/guide/12-stages-1-4.md, "52" in docs/guide/11-worked-examples.md, and "64" in spec/17-crossref.md
...
**Additional instances**: "104 total INVs" (should be 107) in docs/guide/10-verification.md.
...
docs/guide/10-verification.md also says a number that should match. The stale value doesn't
...
- docs/guide/12-stages-1-4.md §12.7 activation list counts to 21 INVs for Stage 2 but
...
**Trigger**: Audit of docs/guide/08-guidance.md found it implements flat procedural rules for
...
rule set grows. Despite this, docs/guide/08-guidance.md defines a `DriftSignals` struct and
...
**Observations**:
- Session 006: docs/guide/08-guidance.md contradicts ADR-GUIDANCE-001 (comonadic topology vs. flat rules)
- docs/guide/08-guidance.md contradicts ADR-GUIDANCE-003 (6 mechanisms — only 2 of 6 align)
- docs/guide/08-guidance.md's GuidanceFooter type has different fields than spec/12-guidance.md
...
**Trigger**: Agent 8's type catalog compilation — enumerating every type definition across spec/ and docs/guide/ — found 13 types where the same concept had incompatible names or field definitions between the two surfaces.
**Status**: TESTABLE
**Severity**: S0 — Structural
**Divergence Type**: ST (Structural)
...
**What happened**: The same conceptual type had different names in spec/ and docs/guide/. Examples:
...
**Observations**:
- V1 Audit: 13 types with incompatible names/definitions between spec/ and docs/guide/
...
**Trigger**: Agent 8's type resolution analysis — attempting to resolve every type reference in function signatures to a formal definition — found 21 types that resolve to no definition in either spec/ or docs/guide/.
**Status**: TESTABLE
**Severity**: S0 — Structural
**Divergence Type**: ST (Structural)
...
fields but never formally defined in either spec/ or docs/guide/. Examples: `AssociateCue`,
...
**Trigger**: Agent 4 compiled the seed section names from spec/06-seed.md, docs/guide/06-seed.md, and docs/guide/00-architecture.md, finding that 3 of 5 section names differed across all three documents despite describing the same template.
**Status**: TESTABLE
...
spec/04-resolution.md. The 24 proptest harnesses in docs/guide/10-verification.md §10.7 pass
...
| 3 | INV-GUIDANCE-009/010 proxy_betweenness() | docs/guide/08-guidance.md:419,448 | WRITTEN |
| 4 | Stratum 0-1 only at Stage 0 | docs/guide/03-query.md:187 | WRITTEN |
| 5 | Cascade steps 2-5 stub datoms | docs/guide/07-merge-basic.md:107 | WRITTEN |
...
docs/HARVEST.md Session 008) propagated the "8 notes" claim without verification.
...
- The "8 notes" claim propagated to: V1_AUDIT_TRIAGE.md D1 section, wave1-findings-resolution.md AV1, docs/HARVEST.md Session 008 — each downstream document amplified the false count
...
spec was revised to add LAYOUT/TRILATERAL namespaces. The V:KANI count in docs/guide/10-verification.md
disagreed with spec/16-verification.md by 7 invariants; function signatures in
docs/guide/13-trilateral.md used types not defined in the spec.
...
docs/guide/10-verification.md disagreed with spec/16-verification.md by 7 invariants; function
signatures in docs/guide/13-trilateral.md used types not defined in the spec. The temporal gap
...
discover that docs/guide/10-verification.md must also be updated. The temporal gap between spec
...
**Observations**:
- docs/guide/10-verification.md V:KANI count disagreed with spec/16-verification.md by 7 invariants after LAYOUT/TRILATERAL additions
- docs/guide/13-trilateral.md function signatures referenced types not defined in the spec
