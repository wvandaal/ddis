...
## Session 020 — Codebase Reorganization: Full Execution

**Date**: 2026-03-09 / 2026-03-10
**Scope**: Complete execution of PROPOSED_CODE_FILE_REORGANIZATION_PLAN.md
**Traces to**: SEED.md §10 (staged roadmap), NEG-008 (no monolithic files), C5 (traceability)

### What Was Accomplished

**Phase A: Performance Optimizations (4 isomorphic refactors)**

Implemented OPT-001 through OPT-004 in `crates/braid-kernel/` — replacing O(n) linear scans
with pre-computed HashSet/HashMap lookups in merge, resolution, store, and proptest_strategies.
All 288 tests used as equivalence oracle; zero behavioral change confirmed.

| Optimization | File | Pattern | Before | After |
|---|---|---|---|---|
| OPT-001 | `merge.rs` | Retraction lookup in merge | O(n) Vec scan | O(1) HashSet |
| OPT-002 | `resolution.rs` | Resolution mode dispatch | O(n) Vec::contains | O(1) HashSet |
| OPT-003 | `store.rs` | Entity-attribute index build | Repeated O(n) scan | O(1) HashMap |
| OPT-004 | `proptest_strategies.rs` | Existing attr filter | O(n²) nested scan | O(1) HashSet |

**Phase B: Reorganization Plan**

Produced `PROPOSED_CODE_FILE_REORGANIZATION_PLAN.md` (283 lines) covering: current state analysis,
design principles, proposed structure, code reorganization, documentation reorganization, file
merge/split recommendations, migration mechanics, breaking change inventory, verification protocol.

**Phase C: Plan Execution (8 phases, 7 beads)**

| Phase | Bead | Description | Status |
|---|---|---|---|
| 1 | brai-33u2 | Create target directory structure (`crates/`, `docs/`, subdirs) | CLOSED |
| 2 | brai-1hg7 | Move Rust crates to `crates/` + update `Cargo.toml` workspace | CLOSED |
| 3 | — | (Merged into Phase 2: Cargo.toml update was atomic with move) | — |
| 4 | brai-1nh6 | Move documentation to `docs/` hierarchy (75 files) | CLOSED |
| 5 | brai-i0ja | Delete empty dirs and stub files (6 items) | CLOSED |
| 6 | brai-2l4q | Create new index documents (`docs/README.md`, `docs/history/README.md`) | CLOSED |
| 7 | brai-2q3g | Update all stale cross-references (~220 refs across ~50 files) | CLOSED |
| 8 | brai-1s62 | Verify all 288 tests + clippy + fmt + final cross-ref audit | CLOSED |

**Phase 7 detail (most complex phase)**: 8 parallel agents updated cross-references across:
- `.github/workflows/kani.yml` — 12 CI path fixes (CRITICAL)
- `AGENTS.md` / `CLAUDE.md` — 22 edits (project structure tree, source docs, session lifecycle)
- `SEED.md` — 6 edits (design doc references)
- `spec/*.md` — 22 edits across 13 files (ADRS.md, guide/, GAP_ANALYSIS.md refs)
- `docs/guide/*.md` — 26 edits across 14 files (crate path updates)
- `docs/design/*.md` — 60+ edits across 3 files (guide/, HARVEST.md refs)
- `docs/audits/*.md` — 20+ edits across 6 files (guide/ refs)
- Rust sources + `docs/history/` — 2 source edits + 15 history edits

**Phase 8 verification results**:
- **288/288 tests pass** (26 unit + 244 proptest + 11 cross-namespace + 3 harvest-seed + 4 stateright)
- **Clippy: 0 warnings** (with `-D warnings`)
- **Fmt: clean** (no formatting issues)
- **Cross-ref audit**: All stale references resolved except intentional historical records

### Quantitative Summary

| Metric | Value |
|---|---|
| Rust files in `crates/` | 41 |
| Rust LOC in `crates/` | 17,912 |
| Documentation files in `docs/` | 88 |
| Specification files in `spec/` | 21 |
| Total files moved/renamed | 143 (git staging) |
| Total cross-reference edits | ~220+ across ~50 files |
| Tests passing | 288/288 |
| Clippy warnings | 0 |
| Root items reduced from | 24 → 6 |
| Beads tracked | 7 phases (all CLOSED) |
| Performance optimizations | 4 (OPT-001..004) |

### Decisions Made

1. **`crates/` convention adopted** (not `lib/` or flat): Standard Rust ecosystem pattern, cargo-compatible, scales to future crates. Rationale in PROPOSED_CODE_FILE_REORGANIZATION_PLAN.md §5.
2. **`docs/` hierarchy**: `docs/design/`, `docs/audits/`, `docs/guide/`, `docs/history/` — separates active design docs from historical archives.
3. **Historical references preserved**: HARVEST.md session entries, raw transcripts, and the reorganization plan itself intentionally keep old paths — they describe historical events.
4. **Within-guide `guide/XX` convention**: ~100+ references in `docs/guide/types.md` use `guide/01-store.md` as documentation labels (not file links). Left as-is since they're self-consistent within the guide directory and are identifiers, not navigable paths.
5. **Spec stays at root**: `spec/` not moved under `docs/` — it's a primary artifact, not documentation.

### Open Questions

1. **Within-guide label convention**: Should `guide/01-store.md` references within guide files be updated to relative `./01-store.md` or fully-qualified `docs/guide/01-store.md`? Low priority — cosmetic only.
2. **PROPOSED_CODE_FILE_REORGANIZATION_PLAN.md**: Should this be moved to `docs/` or kept at root as a completed plan artifact? Currently at root.

### Failure Modes Discovered

- **FM-020: Shell SIGABRT from parallel agent memory pressure** — Running 8 parallel agents caused exit code 134 (SIGABRT) on all subsequent shell commands. The shell subsystem became unresponsive for the remainder of that conversation turn. Mitigation: limit parallel agents to 4-5 in future sessions, or use sequential batching for large parallelized edits.
- **FM-021: Output suppression on crashed shell** — After SIGABRT, commands like `ls`, `date`, `cargo test` would execute (exit 0 or specific error codes) but produce zero stdout/stderr output. This makes diagnosis impossible. Only tool that still worked was `Read` (file-level access, not shell). Recovery required a new conversation.

### Recommended Next Action

1. **Commit and push** all changes (this session's final step).
