...
pub fn format_for_injection(store: &Store, task: Option<&str>, budget: usize) -> String {
    ...
    let now = braid_kernel::now_secs();
    let context = seed::assemble(store, &neighborhood, task_desc, budget, now);
...
pub fn inject_methodology(text: &str, store: &Store, k_eff: f64) -> String {
    let content = guidance::generate_methodology_section(store, k_eff, braid_kernel::now_secs());
