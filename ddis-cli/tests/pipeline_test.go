...
// TestPipelineOnRealSpec runs the pipeline on the actual CLI spec if available.
func TestPipelineOnRealSpec(t *testing.T) {
	manifestPath := "../ddis-cli-spec/manifest.yaml"
	if _, err := os.Stat(manifestPath); err != nil {
		t.Skipf("CLI spec not found at %s", manifestPath)
	}
